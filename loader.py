...
if __name__ == "__main__":
    ...
    # export mixed_audio test file for demo
    from scipy.io.wavfile import write
    ...
    args = {
        ...
        "save_filepath" : "/code/result/trained_model.pt",
        'GPU_NUM' : 0,
    ...
    dataset = CustomDataset(args, train=False)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args["batch_size"], shuffle=True)
    
    for i, (X_train, y_train) in enumerate(dataloader):
        X_write = X_train[0]
        write(f"test_{i}.wav", 44100, X_write.cpu().detach().numpy())
        if i == 9: break